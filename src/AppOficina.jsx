...
// ââ Credenciales oficina ââââââââââââââââââââââââââââââââââââââ
// Para cambiar: edita estas dos constantes
const OFICINA_USER = import.meta.env.VITE_OFICINA_USER || "herrero";
const OFICINA_PASS = import.meta.env.VITE_OFICINA_PASS || "autoescuela2024";
const SESSION_KEY  = "herrero_auth";

function LoginOficina({ onLogin }) {
  ...
  const intentar = () => {
    if (user.trim() === OFICINA_USER && pass === OFICINA_PASS) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F7F3EE", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Segoe UI',sans-serif", padding:20 }}>
      <div style={{ background:"white", borderRadius:18, boxShadow:"0 8px 40px rgba(0,0,0,0.12)", padding:"36px 32px", width:"100%", maxWidth:360 }}>
        {/* Logo */}
        <div style={{ textAlign:"center", marginBottom:24 }}>
          <img src={LOGO_SRC} alt="Herrero" style={{ height:56, objectFit:"contain" }} />
          <div style={{ marginTop:12, fontSize:13, color:"#7A7A7A", fontWeight:600, letterSpacing:"0.5px", textTransform:"uppercase" }}>Panel Oficina</div>
        </div>

        {/* Usuario */}
        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:11, fontWeight:700, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:5 }}>Usuario</div>
          <input
            type="text" value={user} onChange={e=>{setUser(e.target.value);setError("");}}
            onKeyDown={onKey} autoFocus autoComplete="username"
            style={{ width:"100%", border:"1.5px solid #E8E0D5", borderRadius:10, padding:"11px 13px", fontFamily:"inherit", fontSize:14, outline:"none", background:"#F7F3EE", boxSizing:"border-box" }}
          />
        </div>

        {/* ContraseÃ±a */}
        <div style={{ marginBottom:20 }}>
          <div style={{ fontSize:11, fontWeight:700, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:5 }}>ContraseÃ±a</div>
          <div style={{ position:"relative" }}>
            <input
              type={visible?"text":"password"} value={pass} onChange={e=>{setPass(e.target.value);setError("");}}
              onKeyDown={onKey} autoComplete="current-password"
              style={{ width:"100%", border:"1.5px solid #E8E0D5", borderRadius:10, padding:"11px 40px 11px 13px", fontFamily:"inherit", fontSize:14, outline:"none", background:"#F7F3EE", boxSizing:"border-box" }}
            />
            <button onClick={()=>setVisible(v=>!v)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", fontSize:16, color:"#9A9A9A", padding:2 }}>
              {visible?"ð":"ðï¸"}
            </button>
          </div>
        </div>
        ...
        {/* Error */}
        {error && (
          <div style={{ marginBottom:14, padding:"9px 13px", background:"#FFF0F0", borderRadius:8, border:"1px solid #FFCCCC", fontSize:12, color:"#C8102E", fontWeight:600 }}>
            â {error}
          </div>
        )}

        {/* BotÃ³n */}
        <button onClick={intentar} style={{ width:"100%", padding:"13px", background:"#1A3A6B", color:"white", border:"none", borderRadius:12, cursor:"pointer", fontFamily:"inherit", fontSize:14, fontWeight:800, letterSpacing:"0.3px", boxShadow:"0 4px 14px rgba(26,58,107,0.25)" }}>
          Entrar â
        </button>

        <div style={{ marginTop:16, textAlign:"center", fontSize:10, color:"#BBBBBB" }}>
          Autoescuela Herrero Â· Acceso restringido
        </div>
      </div>
    </div>
  );
...
// MOTOR DE ASIGNACIÃN â Autoescuela Herrero
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// MÃ¡ximo hueco permitido entre prÃ¡cticas del mismo profesor (minutos)
const MAX_HUECO_PROF = 30;
// MÃ¡ximo nÃºmero de cortes permitidos por profesor por dÃ­a
const MAX_CORTES_DIA = 1;

// âââ Utilidades de tiempo ââââââââââââââââââââââââââââââââââââ
function toMin(hhmm) {
...
// para un recurso (profesor o vehÃ­culo) en un dÃ­a dado
...
// La pausa se coloca donde hay menos prÃ¡ctica â siempre dentro de 14:00-16:30
function restarPausaComida(tramos, ocupaciones) {
  ...
  // Si ya hay prÃ¡cticas entre 14:00-16:30, adaptar la pausa alrededor de ellas
  ...
  if (ocupEnVentana.length === 0) {
    // Sin prÃ¡cticas en ventana: pausa 14:00-15:30
    pausaDesde = PAUSA_DESDE;
    pausaHasta = PAUSA_DESDE + PAUSA_DUR;
  } else {
    // Hay prÃ¡cticas: poner pausa despuÃ©s de la Ãºltima que termina dentro de la ventana
    ...
    if (pausaHasta > PAUSA_HASTA) {
      // No cabe despuÃ©s: ponerla antes de la primera
...
// IntersecciÃ³n de dos listas de tramos
...
// âââ Obtener huecos libres de duraciÃ³n mÃ­nima ââââââââââââââââ
function huecosSuficientes(tramos, durMin) {
  return tramos.filter(t => (t.hasta - t.desde) >= durMin);
}

// âââ Score de adyacencia (optimizaciÃ³n compacidad) ââââââââââ
// Cuanto mÃ¡s cerca del CAP o de otra prÃ¡ctica ya asignada, mejor
...
// âââ Verificar que la jornada del profesor no se rompe ââââââ
// Devuelve true si colocar una prÃ¡ctica en [desde, hasta] respeta
// la regla de jornada continua (mÃ¡x 1 corte de 30 min al dÃ­a)
function jornadaValida(desde, hasta, ocupaciones) {
  if (ocupaciones.length === 0) return true; // primera prÃ¡ctica del dÃ­a, siempre vÃ¡lida

  // Calcular todos los huecos que habrÃ­a si aÃ±adimos esta prÃ¡ctica
...
// âââ Elegir mejor hueco para una prÃ¡ctica âââââââââââââââââââ
...
// âââ Verificar restricciÃ³n de pista âââââââââââââââââââââââââ
function pistaBloqueada(ocupacionesPista, desde, hasta) {
  return ocupacionesPista.some(oc => oc.hasta > desde && oc.desde < hasta);
}

// âââ MOTOR PRINCIPAL âââââââââââââââââââââââââââââââââââââââââ
/*
  Entradas:
...
function generarPlanning(configSemanal, alumnos, diasSemana) {
  ...
  // Ocupaciones por recurso y dÃ­a: { [profKey_dia]: [{desde,hasta}], [vehKey_dia]: [...] }
  const ocupProf = {};
  const ocupVeh  = {};
  ...
  // AsÃ­ Mamen rellena horas con: B propios â C circ â C pista
  const ordenados = [
  ...
  for (const alumno of ordenados) {
    ...
    // Para alumnos de examen: priorizar dÃ­as ANTERIORES mÃ¡s prÃ³ximos al examen
    // (el dÃ­a mÃ¡s cercano antes del examen es donde mejor prepararlos)
    const diasOrdenados = alumnosExamen.has(alumno.id) && configSemanal.diaExamen
      ? [...diasSemana].sort((a, b) => {
          const oA = DIAS_ORDEN[a] ?? 99;
          const oB = DIAS_ORDEN[b] ?? 99;
          const antA = oA < diaExamenOrden ? diaExamenOrden - oA : 999; // dÃ­as anteriores: distancia positiva pequeÃ±a
          const antB = oB < diaExamenOrden ? diaExamenOrden - oB : 999;
          return antA - antB; // mÃ¡s cercano anterior primero; dÃ­as posteriores van al final
        })
      : diasSemana;
    ...
    for (const dia of diasOrdenados) {
      ...
      // Tramos disponibles del alumno ese dÃ­a
      ...
      // Bloqueo examen: si hay examen este dÃ­a, bloquear franja 09:00-14:00
      ...
      // LÃ­mite de pista ese dÃ­a
      const limitePista = toMin(configSemanal.horasPista?.[dia] || HORA_MAX);

      // CuÃ¡ntas prÃ¡cticas puede tener el alumno este dÃ­a
      const maxHoy = alumno.permiso === "B" ? 2 : 1; // B: mÃ¡x 2 seguidas/dÃ­a Â· C y C+E: mÃ¡x 1/dÃ­a
      let asignadasHoy = 0;

      // Intentar asignar maxHoy prÃ¡cticas este dÃ­a
      while (asignadasHoy < maxHoy && asignadas < maxSemanal) {
        let asignado = false;

        // ââ Prioridad de profesores ââââââââââââââââââââââââââââââââââââââââââ
        // Mamen: mÃ­nimo calculado segÃºn tramos disponibles ese dÃ­a:
        //   maÃ±ana+tarde disponible = 8h | solo maÃ±ana = 5h | solo tarde = 4h
        //   dÃ­a con examen = descuenta el bloqueo | no disponible = 0 (sin asignar)
        // Relleno si no llega con B propios: C circ â C pista (en ese orden)
        ...
        // Calcular mÃ­nimo de Mamen segÃºn tramos disponibles hoy
        let minMamen = 0;
        ...
        // C: si Mamen necesita horas â Mamen primero; si no â Pablo primero
          ...
          if (alumno.permiso !== "B") {
            ...
            for (const vehKey of vehCompatibles) {
              ...
              if (esPista) {
                ...
                // No puede haber otro vehÃ­culo en pista simultÃ¡neamente
            ...
            // MÃ³dulo B: no necesita vehÃ­culo especÃ­fico
            // Si es la 2Âª prÃ¡ctica del dÃ­a, DEBE ser contigua a la 1Âª (optimizaciÃ³n)
            let tramosParaB = tramosComunes;
            if (asignadasHoy >= 1) {
              // Buscar la prÃ¡ctica anterior del mismo alumno en este dÃ­a
              const pracAnterior = planning[dia]
                .filter(p => p.alumnoId === alumno.id)
                .sort((a, b) => toMin(b.hasta) - toMin(a.hasta))[0]; // la mÃ¡s reciente
              if (pracAnterior) {
                const inicioContiguo = toMin(pracAnterior.hasta);
                // Solo aceptar el tramo que empieza exactamente donde terminÃ³ la anterior
        ...
        if (!asignado) break; // No hay mÃ¡s huecos este dÃ­a
      }
  ...
  // Ordenar cada dÃ­a por hora de inicio
...
// âââ Ampliar prÃ¡ctica a 60 min (acciÃ³n de oficina) âââââââââââ
function ampliarPractica(planning, dia, index, ocupProf, ocupVeh, ocupPista) {
  const practica = planning[dia][index];
  if (!practica) return { ok: false, motivo: "PrÃ¡ctica no encontrada" };
  ...
  if (nuevaHasta > limiteMax) return { ok: false, motivo: "Supera el lÃ­mite de " + HORA_MAX };

  // Verificar que no hay colisiÃ³n con siguiente prÃ¡ctica del profesor o vehÃ­culo
  ...
  if (colision) return { ok: false, motivo: "ColisiÃ³n con otra prÃ¡ctica del profesor" };
...
// Formato duraciÃ³n: 30 â "30min" Â· 60 â "1h" Â· 90 â "1h30min"
...
// Fusionar prÃ¡cticas contiguas del mismo alumno en el planning
// Se llama DESPUÃS de generar, en el componente
const VEH_LABEL   = { renault_amarillo:"R.Amarillo(C)", renault_blanco:"R.Blanco(C)", trailer_renault:"TrÃ¡iler R.(C+E)", trailer_mercedes:"TrÃ¡iler M.(C+E)", audi_a3:"Audi A3", toyota_auris:"Toyota Auris" };
const VEHICULOS_PESADOS = ["renault_amarillo","renault_blanco","trailer_renault","trailer_mercedes"];

// ââ Estado inicial config semanal âââââââââââââ
...
// ââ Alumnos demo ââââââââââââââââââââââââââââââ
const ALUMNOS_DEMO = [
  { id:1, nombre:"Antonio", apellidos:"FernÃ¡ndez Mora",   telefono:"634112233", localidad:"Trujillo",   permiso:"C",   fase:"pista",       activo:true,  bono:false, bonoRestantes:null, fechaAlta:"2024-09-01", profesorFijo:null,    cocheAsignado:null },
  { id:2, nombre:"Carlos",  apellidos:"SÃ¡nchez DÃ­az",     telefono:"698556677", localidad:"La Cumbre",  permiso:"C+E", fase:"circulacion",  activo:true,  bono:true,  bonoRestantes:6,    fechaAlta:"2024-11-03", profesorFijo:null,    cocheAsignado:null },
  ...
  { id:4, nombre:"JosÃ©",    apellidos:"PÃ©rez Alonso",     telefono:"655001122", localidad:"Ibahernando",permiso:"C",   fase:"circulacion",  activo:true,  bono:true,  bonoRestantes:3,    fechaAlta:"2025-01-10", profesorFijo:null,    cocheAsignado:null },
  { id:5, nombre:"MarÃ­a",   apellidos:"GarcÃ­a LÃ³pez",     telefono:"677889900", localidad:"Trujillo",   permiso:"B",   fase:null,           activo:true,  bono:false, bonoRestantes:null, fechaAlta:"2024-10-15", profesorFijo:"mamen", cocheAsignado:"audi_a3",      maxPracticas:6 },
  { id:6, nombre:"LucÃ­a",   apellidos:"MartÃ­n Rubio",     telefono:"611223344", localidad:"Trujillo",   permiso:"B",   fase:null,           activo:true,  bono:false, bonoRestantes:null, fechaAlta:"2024-08-20", profesorFijo:"javi",  cocheAsignado:"toyota_auris", maxPracticas:6 },
  { id:7, nombre:"Miguel",  apellidos:"Romero Castillo",  telefono:"699445566", localidad:"MadroÃ±era",  permiso:"B",   fase:null,           activo:false, bono:false, bonoRestantes:null, fechaAlta:"2025-01-22", profesorFijo:null,    cocheAsignado:null,           maxPracticas:4, transporte:true },
];
...
function SelectorEstado({ valor, onChange }) {
  return (
    <div style={{ display:"flex", gap:4 }}>
      {[{v:"todo",l:"Todo el dÃ­a"},{v:"tramos",l:"Por tramos"},{v:"no",l:"No disponible"}].map(op=>(
    ...
    <div style={{ marginTop:8 }}>
      {tramos.map((t,i)=>(
        <div key={i} style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6 }}>
          ...
          <span style={{ fontSize:12, color:"#9A9A9A" }}>â</span>
          ...
          <button onClick={()=>onChange(tramos.filter((_,j)=>j!==i))} style={{ width:28, height:28, borderRadius:"50%", border:"1px solid #F5C4C4", background:"#FDF5F5", color:"#C8102E", cursor:"pointer", fontSize:14 }}>Ã</button>
        </div>
      ))}
      <button onClick={()=>onChange([...tramos,{desde:"09:00",hasta:"14:00"}])} style={{ width:"100%", padding:"7px", borderRadius:8, border:"1.5px dashed #1A3A6B44", background:"#EEF3FB", color:"#1A3A6B", fontFamily:"inherit", fontSize:12, fontWeight:600, cursor:"pointer" }}>+ AÃ±adir tramo</button>
    </div>
  );
}

// ââââââââââââââââââââââââââââââââââââââââââââââ
// MÃDULO 1: CONFIGURACIÃN SEMANAL
// ââââââââââââââââââââââââââââââââââââââââââââââ
function ModuloConfig({ cfg, setCfg, alumnos, configId, setConfigId, tokens, setTokens, onNuevaSemana }) {
  ...
  const iniciarNuevaSemana = async () => {
    ...
    try {
      await resetNuevaSemana();
      if (onNuevaSemana) onNuevaSemana();
  ...
  const TABS = [{k:"general",l:"ð General"},{k:"profesores",l:"ð¤ Profesores"},{k:"vehiculos",l:"ð VehÃ­culos"},{k:"pista",l:"ð Pista"},{k:"examen",l:"ð Examen"}];
    ...
    <div>
      ...
      {seccion==="general" && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div>
            <div style={{ fontSize:11, fontWeight:600, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:5 }}>Fecha lÃ­mite alumnos</div>
            <div style={{ display:"flex", gap:8 }}>
          ...
          <div>
            <div style={{ fontSize:11, fontWeight:600, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:8 }}>DÃ­a de examen</div>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {[{v:null,l:"Sin examen"},...DIAS_SEMANA.map(d=>({v:d,l:DIAS_LABEL[d]})),{v:"lunes_sig",l:"Lunes (sem.sig.)"},{v:"martes_sig",l:"Martes (sem.sig.)"}].map(op=>(
                <button key={op.v||"none"} onClick={()=>setP("diaExamen",op.v)} style={{ padding:"7px 12px", borderRadius:8, cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:600, border:"1.5px solid "+(cfg.diaExamen===op.v?(op.v?"#C8102E":"#1A3A6B"):"#E8E0D5"), background:cfg.diaExamen===op.v?(op.v?"#C8102E":"#1A3A6B"):"white", color:cfg.diaExamen===op.v?"white":"#7A7A7A" }}>{op.v?"ð« ":""}{op.l}</button>
              ))}
            </div>
          </div>
          <div>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            }} style={{ flex:1, padding:12, borderRadius:10, border:"1.5px solid #1A3A6B", background:"white", color:"#1A3A6B", fontFamily:"inherit", fontSize:13, fontWeight:700, cursor:"pointer" }}>{guardandoParcial?"â Guardado":"ð¾ Guardar borrador"}</button>
            <button onClick={async()=>{
              try {
                const saved = await guardarConfigBorrador(cfg);
            ...
            }} style={{ flex:2, padding:12, borderRadius:10, border:"none", background:"#C8102E", color:"white", fontFamily:"inherit", fontSize:13, fontWeight:700, cursor:"pointer", boxShadow:"0 4px 14px rgba(200,16,46,0.25)" }}>â¡ Activar semana</button>
          </div>

          {/* BOTÃN RESET NUEVA SEMANA */}
          <button onClick={iniciarNuevaSemana} disabled={reseteando} style={{ width:"100%", marginTop:10, padding:"11px", background:confirmReset?"#8B0000":"#1A1A1A", color:confirmReset?"#FFAAAA":"#FF6B6B", border:"1.5px dashed "+(confirmReset?"#FFAAAA":"#FF6B6B"), borderRadius:12, cursor:reseteando?"not-allowed":"pointer", fontFamily:"inherit", fontSize:11, fontWeight:700, letterSpacing:"0.5px", transition:"all 0.2s" }}>
            {reseteando?"â³ Borrando todo...": confirmReset?"â ï¸ CONFIRMAR: Esto borrarÃ¡ TODOS los datos. Pulsa de nuevo (5s)": "ðï¸ NUEVA SEMANA â Borrar todo y empezar desde cero"}
          </button>

          {/* MODAL SELECCIÃN ALUMNOS Y GENERACIÃN DE TOKENS */}
          {mostrarTokens && (
            <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:400, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
              <div style={{ background:"white", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:500, maxHeight:"85vh", overflowY:"auto", padding:"24px 20px 40px" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
                  <div style={{ fontSize:17, fontWeight:700 }}>ð² Seleccionar alumnos</div>
                  <button onClick={()=>setMostrarTokens(false)} style={{ width:32, height:32, borderRadius:"50%", border:"1.5px solid #E8E0D5", background:"white", cursor:"pointer", fontSize:16 }}>â</button>
                </div>
                <div style={{ fontSize:13, color:"#5A5A5A", marginBottom:14 }}>Elige a quÃ© alumnos enviar el enlace de disponibilidad esta semana.</div>
                <button onClick={()=>setSeleccionados(new Set(alumnos.filter(a=>a.activo).map(a=>a.id)))} style={{ fontSize:12, color:"#1A3A6B", fontWeight:600, background:"none", border:"none", cursor:"pointer", marginBottom:10 }}>â Seleccionar todos</button>
                    ...
                    return (
                      <div key={a.id} onClick={()=>{
                        ...
                        <div style={{ width:22, height:22, borderRadius:6, border:"2px solid "+(sel?"#1A3A6B":"#C0C0C0"), background:sel?"#1A3A6B":"white", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                          {sel && <span style={{ color:"white", fontSize:13, fontWeight:700 }}>â</span>}
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:14, fontWeight:700 }}>{a.apellidos}, {a.nombre}</div>
                          <div style={{ fontSize:11, color:"#7A7A7A" }}>{a.localidad} Â· {a.permiso}</div>
                        </div>
                        {yaToken && <span style={{ fontSize:10, color:yaToken.usado?"#2E7D32":"#E65100", fontWeight:600 }}>{yaToken.usado?"â Enviado":"â³ Pendiente"}</span>}
                      </div>
                    );
                  })}
                </div>
                <button onClick={async()=>{
                  ...
                  try {
                    ...
                    alert(`â ${data.length} enlace(s) generado(s). Ve a la pestaÃ±a WhatsApp para enviarlos.`);
                  } catch(e) {
                  ...
                  {generandoTokens?"â³ Generando...":"ð² Generar enlaces"}
                </button>
              </div>
            </div>
          )}
        ...
        <div>
          <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:14 }}>
            {VEHICULOS_PESADOS.map(vk=>(
              <button key={vk} onClick={()=>setVehActivo(vk)} style={{ padding:"10px 14px", borderRadius:10, cursor:"pointer", textAlign:"left", fontFamily:"inherit", fontSize:13, fontWeight:600, border:"1.5px solid "+(vehActivo===vk?"#1A3A6B":"#E8E0D5"), background:vehActivo===vk?"#1A3A6B":"white", color:vehActivo===vk?"white":"#1C1C1C" }}>ð {VEH_LABEL[vk]}</button>
            ))}
          </div>
          {DIAS_SEMANA.map(dia=>{
      ...
      {seccion==="examen" && (
        <div>
          {!cfg.diaExamen ? (
            <div style={{ background:"#F7F3EE", borderRadius:12, border:"1.5px dashed #E8E0D5", padding:24, textAlign:"center", color:"#9A9A9A", fontSize:13 }}>Configura primero un dÃ­a de examen en <strong>General</strong></div>
          ) : (
            <div>
              <div style={{ background:"#FEF3E2", border:"1.5px solid #F5C47A", borderRadius:12, padding:"10px 14px", marginBottom:14, fontSize:13, color:"#8A5A00" }}>
                ð Examen el <strong>{cfg.diaExamen?.endsWith('_sig') ? (cfg.diaExamen==='lunes_sig'?'Lunes sem.sig.':'Martes sem.sig.') : DIAS_LABEL[cfg.diaExamen]}</strong> â los marcados tienen prioridad y el motor les asigna el dÃ­a mÃ¡s cercano al examen
              </div>
              {alumnos.filter(a=>a.activo).map(a=>{
                ...
                return (
                  <div key={a.id} onClick={()=>setCfg(p=>{const s=new Set(p.alumnosExamen);sel?s.delete(a.id):s.add(a.id);return{...p,alumnosExamen:[...s]};})} style={{ display:"flex", alignItems:"center", gap:12, background:"white", borderRadius:12, marginBottom:8, border:"1.5px solid "+(sel?"#C8102E":"#E8E0D5"), padding:"12px 14px", cursor:"pointer" }}>
                    <div style={{ width:34, height:34, borderRadius:"50%", background:sel?"#C8102E":"#F0EBE5", color:sel?"white":"#9A9A9A", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700 }}>{sel?"â":a.nombre[0]+a.apellidos[0]}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:14, fontWeight:700 }}>{a.apellidos}, {a.nombre}</div>
                      <Badge color={cp}>{a.permiso}</Badge>
                    </div>
                    {sel && <span style={{ fontSize:11, fontWeight:700, color:"#C8102E" }}>ð Examen</span>}
                  </div>
                );
              })}
            </div>
...
// ââââââââââââââââââââââââââââââââââââââââââââââ
// MÃDULO 2: GESTIÃN DE ALUMNOS (compacto)
// ââââââââââââââââââââââââââââââââââââââââââââââ
const LOCALIDADES = ["Trujillo","La Cumbre","Ibahernando","Torrecillas de la Tiesa","Aldeacentenera","GarcÃ­az","MadroÃ±era","Zarza de MontÃ¡nchez","Herguijuela","Santa Cruz de la Sierra","Puerto de Santa Cruz","Otra"];
...
function ModuloAlumnos({ alumnos, setAlumnos }) {
  ...
  // Importar Excel â lee filas desde fila 4 (tras cabecera y ayuda)
  const importarExcel = async (e) => {
    ...
    try {
      ...
      for (const row of dataRows) {
        ...
        // C y C+E â localidad siempre Trujillo
        const localidad = (permiso === "B")
          ? (row[3]||"Trujillo").toString().trim()
          : "Trujillo";

        // VehÃ­culo automÃ¡tico segÃºn permiso+fase
        let cocheAsignado = null;
        if (permiso === "B") {
          cocheAsignado = vehExcel || null; // lo que eligiÃ³ en Excel
        } else if (permiso === "C") {
        ...
        // Normalizar nombre vehÃ­culo B a clave interna
      ...
      if (nuevos.length > 0) {
        ...
        alert(`â ${nuevos.length} alumno(s) importado(s) correctamente.`);
      } else {
        alert("No se encontraron alumnos vÃ¡lidos en el Excel. Comprueba que los datos empiezan en la fila 4.");
      }
    } catch(err) {
      console.error(err);
  ...
  return (
    <div>
      ...
      <div style={{ background:"white", borderRadius:12, border:"1.5px solid #E8E0D5", display:"flex", alignItems:"center", gap:10, padding:"10px 14px", marginBottom:10 }}>
        <span style={{ fontSize:16, color:"#9A9A9A" }}>ð</span>
          ...
          <div key={a.id} style={{ background:"white", borderRadius:12, border:"1.5px solid #E8E0D5", padding:"12px 14px", marginBottom:8, opacity:a.activo?1:0.55 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <button
                onClick={async (e)=>{
                  e.stopPropagation();
                  if (!window.confirm(`Â¿Borrar a ${a.nombre} ${a.apellidos} permanentemente? Esta acciÃ³n no se puede deshacer.`)) return;
                  try {
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", zIndex:300, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
          <div style={{ background:"white", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:480, maxHeight:"90vh", overflowY:"auto", padding:"24px 20px 40px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
              ...
              <button onClick={()=>{setModal(null);setAlumnoEditar(null);}} style={{ width:32, height:32, borderRadius:"50%", border:"1.5px solid #E8E0D5", background:"white", cursor:"pointer", fontSize:16 }}>â</button>
            </div>
...
function FormAlumnoCompacto({ alumno, onGuardar }) {
  ...
  return (
    <div>
      {[{l:"Nombre",k:"nombre"},{l:"Apellidos",k:"apellidos"},{l:"TelÃ©fono",k:"telefono",t:"tel"}].map(f=>(
      ...
      {form.permiso!=="B" && (
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {[{k:"pista",l:"ð Pista"},{k:"circulacion",l:"ð£ï¸ CirculaciÃ³n"}].map(f=>(
      ...
      {!esNuevo && (
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {[{v:true,l:"â Activo"},{v:false,l:"ð Archivado"}].map(op=>(
      ...
      {/* VehÃ­culo asignado */}
      <div style={{ marginBottom:12 }}>
        <div style={{ fontSize:11, fontWeight:600, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:8 }}>VehÃ­culo {form.permiso==="B"?"fijo":"preferente"}</div>
        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
          {[{v:null,l:"Auto"}, ...(form.permiso==="B"
            ...
            : form.permiso==="C"
              ...
              : [{v:"trailer_renault",l:"TrÃ¡iler R."},{v:"trailer_mercedes",l:"TrÃ¡iler M."}]
          )].map(op=>(
...
function generarPDFGeneral(planning, alumnos, cfg) {
  ...
  let html = `<div class="cabecera"><h1>AUTOESCUELA HERRERO â Planning General</h1><p>${semanaLabel(cfg)}</p></div>`;
  for (const profKey of ["mamen","javi","pablo","toni"]) {
    ...
    html += `<h2 style="color:${cp};border-color:${cp}">ð¤ ${PROF_LABEL[profKey]} <span style="font-size:11px;font-weight:400">${todasProf.length} prÃ¡cticas Â· ${Math.floor(totalMin/60)}h ${totalMin%60}min</span></h2>`;
    for (const dia of DIAS_SEMANA) {
      ...
      for (const p of pracs) {
        html += `<div class="practica" style="border-left-color:${cp}">
          <div class="hora">${p.desde} Â· ${durLabel(p.duracion)}</div>
          <div class="info">
            <div class="nombre">${p.alumnoNombre}</div>
            <div class="detalle">${chipPermiso(p.permiso)} ${VEH_LABEL[p.vehiculo]||"â"} Â· ${p.tipo==="pista"?"ð Pista":"CirculaciÃ³n"}</div>
          </div>
        </div>`;
      }
    }
...
function generarPDFsProfesores(planning, alumnos, cfg) {
  ...
  for (const profKey of ["mamen","javi","pablo","toni"]) {
    ...
    const totalMin = todasProf.reduce((a,p)=>a+(p.duracion||0),0);
    const cp = colorProf(profKey);
    let html = `<div class="cabecera" style="background:${cp}"><h1>AUTOESCUELA HERRERO Â· ${PROF_LABEL[profKey].toUpperCase()}</h1><p>${semanaLabel(cfg)}</p></div>`;
    html += `<div class="total">Total semana: ${todasProf.length} prÃ¡cticas Â· ${Math.floor(totalMin/60)}h ${totalMin%60>0?totalMin%60+"min":""}</div>`;
    for (const dia of DIAS_SEMANA) {
      const pracs = (planning[dia]||[]).filter(p=>p.profesor===profKey);
      if (!pracs.length) continue;
...
function generarPDFsAlumnos(planning, alumnos, cfg) {
  ...
  for (const alumno of alumnosConPracs) {
    ...
    let html = `<div class="cabecera" style="background:#C8102E"><h1>AUTOESCUELA HERRERO</h1><p>Tus prÃ¡cticas Â· ${semanaLabel(cfg)}</p></div>`;
    html += `<div style="background:#F7F3EE;border-radius:8px;padding:12px 14px;margin-bottom:14px;">
      ...
      <div style="font-size:11px;color:#666;margin-top:4px">${alumno.localidad} Â· Permiso ${alumno.permiso}${alumno.fase ? " Â· " + alumno.fase : ""}</div>
    </div>`;
    html += `<div class="total">Total semana: ${todasAlumno.length} prÃ¡cticas Â· ${Math.floor(totalMin/60)}h ${totalMin%60>0?totalMin%60+"min":""}</div>`;
    for (const dia of DIAS_SEMANA) {
      ...
      for (const p of pracs) {
        ...
        html += `<div class="practica" style="border-left-color:${cp}">
          ...
          <div class="info">
            ...
            <div class="detalle">${p.desde} Â· ${durLabel(p.duracion)} Â· ${VEH_LABEL[p.vehiculo]||"â"} Â· ${p.tipo==="pista"?"ð Pista":"CirculaciÃ³n"}</div>
          </div>
        </div>`;
      }
    }
    html += `<div class="separador"></div><p style="font-size:10px;color:#999;text-align:center">Autoescuela Herrero Â· C/ TenerÃ­as 6 bajo Â· Trujillo Â· 688 70 86 69</p>`;
    abrirVentanaPDF(`PrÃ¡cticas ${alumno.apellidos} ${alumno.nombre}`, html);
  }
}
...
function ChipPractica({ p, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ background:"white", borderRadius:8, borderLeft:"3px solid "+cp, border:"1px solid "+cp+"33", borderLeftWidth:3, borderLeftColor:cp, padding:"8px 10px", marginBottom:4, cursor:"pointer" }}>
      <div style={{ fontSize:11, fontWeight:700, color:cp }}>{p.desde} <span style={{ fontSize:10, color:"#9A9A9A", fontWeight:600 }}>Â· {durLabel(p.duracion)}</span></div>
      ...
      <div style={{ display:"flex", gap:4, marginTop:4, flexWrap:"wrap" }}>
        ...
        {p.tipo==="pista"&&<span style={{ fontSize:10, color:"#D4700A", fontWeight:600 }}>ðPista</span>}
      </div>
    </div>
  );
}
...
function ModuloPlanning({ cfg, alumnos, configId, planning, setPlanning, sinAsignar, setSinAsignar }) {
  ...
  const franjaATramos = (franjas) => {
    const mapa = { manana:[{desde:"09:00",hasta:"14:00"}], mediodia:[{desde:"14:00",hasta:"17:00"}], tarde:[{desde:"17:00",hasta:"20:30"}] }; // noche eliminada â tarde cubre 14-21h
    return franjas.flatMap(f => mapa[f] || []);
  };

  const totalAsignadas = planning ? Object.values(planning).flat().length : 0;

  // ââ FusiÃ³n de prÃ¡cticas contiguas (en render, inmune a tree-shaking) ââ
  // FusiÃ³n: usa funciÃ³n exportada desde supabase.js (inmune a tree-shaking de Vite)
  const planningFusionado = useMemo(() => fusionarPlanning(planning), [planning]);


  return (
    <div>
      <button onClick={ejecutar} disabled={generando} style={{ width:"100%", padding:14, background:generando?"#5A5A5A":"#1A3A6B", color:"white", border:"none", borderRadius:12, fontFamily:"inherit", fontSize:15, fontWeight:700, cursor:generando?"not-allowed":"pointer", boxShadow:"0 4px 16px rgba(26,58,107,0.3)", marginBottom:12 }}>{generando ? "â³ Generando..." : "â¡ Generar planning"}</button>
      ...
      {planningFusionado && <>
        ...
        {sinAsignar.length>0 && (
          <div style={{ background:"#FDF5F5", border:"1.5px solid #F5C4C4", borderRadius:10, padding:"10px 14px", marginBottom:12 }}>
            <div style={{ fontSize:12, fontWeight:700, color:"#C8102E", marginBottom:6 }}>â ï¸ Sin asignar</div>
            {sinAsignar.map((a,i)=><div key={i} style={{ fontSize:12, color:"#5A5A5A" }}>Â· {a.alumnoNombre} ({a.permiso})</div>)}
          </div>
        )}

        {/* Botones de PDF */}
        <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" }}>
          <button onClick={()=>generarPDFGeneral(planning, alumnos, cfg)} style={{ flex:"1 1 auto", padding:"9px 12px", borderRadius:10, border:"1.5px solid #1A3A6B", background:"white", color:"#1A3A6B", fontFamily:"inherit", fontSize:12, fontWeight:700, cursor:"pointer" }}>ð PDF General</button>
          <button onClick={()=>generarPDFsProfesores(planning, alumnos, cfg)} style={{ flex:"1 1 auto", padding:"9px 12px", borderRadius:10, border:"1.5px solid #2E7D32", background:"white", color:"#2E7D32", fontFamily:"inherit", fontSize:12, fontWeight:700, cursor:"pointer" }}>ð¤ PDF por Profesor</button>
          <button onClick={()=>generarPDFsAlumnos(planning, alumnos, cfg)} style={{ flex:"1 1 auto", padding:"9px 12px", borderRadius:10, border:"1.5px solid #C8102E", background:"white", color:"#C8102E", fontFamily:"inherit", fontSize:12, fontWeight:700, cursor:"pointer" }}>ð PDF por Alumno</button>
        </div>

        <div style={{ display:"flex", gap:6, marginBottom:10, overflowX:"auto" }}>
          {[{k:"dia",l:"ð Por dÃ­a"},{k:"profesor",l:"ð¤ Profesor"},{k:"semanal",l:"ð Semanal"},{k:"vehiculos",l:"ð VehÃ­culos"}].map(v=>(
          ...
          return (
            <div key={pk} style={{ background:"white", borderRadius:12, border:"1.5px solid "+cp+"33", marginBottom:10, overflow:"hidden" }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, background:cp+"12", padding:"10px 14px", borderBottom:"1px solid "+cp+"22" }}>
                ...
                <div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:700 }}>{PROF_LABEL[pk]}</div><div style={{ fontSize:11, color:"#9A9A9A" }}>{pracs.length} prÃ¡cticas</div></div>
              </div>
              <div style={{ padding:"8px 10px" }}>
                {pracs.length===0?<div style={{ fontSize:12, color:"#C0C0C0", textAlign:"center", padding:"8px 0" }}>Sin prÃ¡cticas</div>:pracs.map((p,i)=><ChipPractica key={i} p={p} onClick={()=>setModalP({p,dia:diaActivo})} />)}
              </div>
            </div>
          );
        })}
              ...
              return (
                <div key={vk} style={{ background:"white", borderRadius:12, border:"1.5px solid #E8E0D5", marginBottom:12, overflow:"hidden" }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"#1A3A6B", padding:"10px 14px" }}>
                    <div style={{ color:"white", fontWeight:700, fontSize:14 }}>ð {VEH_LABEL[vk]}</div>
                    <div style={{ color:"#A8C4E8", fontSize:12, fontWeight:600 }}>{totalH}h {totalM>0?totalM+"min":""} Â· {todasPracs.length} prÃ¡cticas</div>
                  </div>
                  {DIAS_SEMANA.map(d => {
                        ...
                        {pracs.map((p,i) => (
                          <div key={i} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, padding:"6px 10px", background:"#F7F3EE", borderRadius:8, borderLeft:"3px solid "+(COLOR_PROF[p.profesor]||"#888") }}>
                            <div style={{ flex:1 }}>
                              ...
                              <div style={{ fontSize:11, color:"#7A7A7A" }}>{p.desde} Â· {durLabel(p.duracion)} Â· {PROF_LABEL[p.profesor]}</div>
                            </div>
                            <div style={{ display:"flex", gap:4 }}>
                              {p.tipo==="pista" && <span style={{ fontSize:10, color:"#D4700A", fontWeight:700 }}>ðPista</span>}
        ...
        {modalP && (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", zIndex:400, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
            <div style={{ background:"white", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:480, padding:"24px 20px 40px", maxHeight:"90vh", overflowY:"auto" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
                <div style={{ fontSize:17, fontWeight:700 }}>Editar prÃ¡ctica</div>
                <button onClick={()=>setModalP(null)} style={{ width:32, height:32, borderRadius:"50%", border:"1.5px solid #E8E0D5", background:"white", cursor:"pointer", fontSize:16 }}>â</button>
              </div>
              <div style={{ background:COLOR_PROF[modalP.p.profesor]+"11", border:"1px solid "+COLOR_PROF[modalP.p.profesor]+"33", borderRadius:12, padding:"14px 16px", marginBottom:16 }}>
                ...
                <div style={{ fontSize:14, color:COLOR_PROF[modalP.p.profesor], fontWeight:700, marginTop:4 }}>{DIAS_LABEL[modalP.dia]} Â· {modalP.p.desde} Â· {durLabel(modalP.p.duracion)}</div>
                <div style={{ display:"flex", gap:6, marginTop:8 }}>
                  <Badge color={COLOR_PERM[modalP.p.permiso]}>{modalP.p.permiso}</Badge>
                  {modalP.p.tipo==="pista"&&<Badge color="#D4700A">ð Pista</Badge>}
                  <Badge color="#5A7A9A">ð {VEH_LABEL[modalP.p.vehiculo]||"Sin vehÃ­culo"}</Badge>
                </div>
              </div>
              {/* Cambiar profesor */}
              <div style={{ marginBottom:14 }}>
              ...
              {/* Cambiar vehÃ­culo */}
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:8 }}>Cambiar vehÃ­culo</div>
                <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                  {(modalP.p.permiso==="B"
                ...
                {/* Cambiar dÃ­a y hora */}
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>ð Mover a otro dÃ­a</div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    {["lunes","martes","miercoles","jueves","viernes"].map(d=>(
                      <button key={d} onClick={()=>setModalP(mp=>({...mp, dia:d}))}
                        ...
                        {({lunes:"Lun",martes:"Mar",miercoles:"MiÃ©",jueves:"Jue",viernes:"Vie"})[d]}
                      </button>
                    ))}
                  </div>
                </div>
                <div style={{ marginBottom:16 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"#5A5A5A", textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>ð Cambiar hora inicio</div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                <button onClick={()=>{
                  ...
                  setModalP(null);
                }} style={{ padding:12, borderRadius:12, border:"1.5px solid #1A3A6B33", background:"#F0F4FF", color:"#1A3A6B", fontFamily:"inherit", fontSize:14, fontWeight:600, cursor:"pointer" }}>ð¾ Guardar cambios</button>
                <button onClick={()=>{setPlanning(p=>{const n={...p};const orig=modalP.diaOrig||modalP.dia;n[orig]=(n[orig]||[]).filter(x=>x!==modalP.p);return n;});setModalP(null);}} style={{ padding:12, borderRadius:12, border:"1.5px solid #C8102E33", background:"#FDF5F5", color:"#C8102E", fontFamily:"inherit", fontSize:14, fontWeight:600, cursor:"pointer" }}>ð Eliminar prÃ¡ctica</button>
              </div>
            </div>
          </div>
        )}
...
// ââââââââââââââââââââââââââââââââââââââââââââââ
// MÃDULO 4: WHATSAPP (compacto)
// ââââââââââââââââââââââââââââââââââââââââââââââ

// ââââââââââââââââââââââââââââââââââââââââââââââ
// MÃDULO RESPUESTAS
// ââââââââââââââââââââââââââââââââââââââââââââââ
// ââ PDF RESUMEN RESPUESTAS ââââââââââââââââââââââââââââââââââââ
function generarPDFRespuestas(disponibilidades, tokensLocales, alumnos, cfg) {
  const DIAS_L  = { lunes:"Lunes", martes:"Martes", miercoles:"MiÃ©rcoles", jueves:"Jueves", viernes:"Viernes", sabado:"SÃ¡bado" };
  const FRANJAS_L = { manana:"MaÃ±ana (9-14h)", mediodia:"Mediodía (14-17h)", tarde:"Tarde (17-20:30h)" };
  ...
  // Mapa alumnoId â ficha completa
  const alumnoMap = {};
  ...
  <title>Respuestas Alumnos â ${semanaLabel(cfg)}</title>
  <style>
  ...
  html += `<div class="header">
    <div><h1>ð¬ Respuestas de Disponibilidad</h1><p>${semanaLabel(cfg)}</p></div>
    <div class="header-right">Generado: ${fechaGen}<br>Autoescuela Herrero Â· 688 70 86 69</div>
  </div>`;
  ...
  // SecciÃ³n respondidos
  if (dispsConAlumno.length > 0) {
    html += `<div class="seccion-titulo">â Han respondido (${dispsConAlumno.length})</div>`;
    dispsConAlumno.forEach(d => {
      const a = d.alumno;
      const nombre = (a.apellidos||"?") + ", " + (a.nombre||"?");
      const meta = [a.localidad, "Permiso " + (a.permiso||"?"), a.fase].filter(Boolean).join(" Â· ");
      const transporte = a.transporte;
      const deseadas = d.practicas_deseadas || "â";
      const dias = d.dias || {};

      // Calcular dÃ­as con disponibilidad
      ...
      html += `<div class="alumno-card">
        <div class="alumno-head">
          <div>
            ...
            <div class="alumno-meta">${meta}${transporte ? '<span class="transporte-badge">ð Transporte</span>' : ""}</div>
          </div>
          <div style="text-align:right">
            <span class="deseadas">ð¯ ${deseadas} prÃ¡ctica${deseadas > 1 ? "s" : ""}</span>
          </div>
        </div>
        <div class="alumno-body">`;

      if (diasConFranjas.length === 0) {
        html += `<span class="sin-respuesta">Sin dÃ­as disponibles indicados</span>`;
      } else {
        html += `<div class="dias-grid">`;
        DIAS_SEMANA.forEach(dia => {
          const franjas = dias[dia] || [];
          if (!franjas.length) return;
          // Color del dÃ­a segÃºn nÃºmero de franjas
  ...
  if (pendientes.length > 0) {
    html += `<div class="seccion-titulo">â³ Pendientes de respuesta (${pendientes.length})</div>
  ...
  html += `<div class="footer">Autoescuela Herrero Â· C/ TenerÃ­as 6 bajo Â· Trujillo Â· 688 70 86 69 Â· herreroformacion.es</div></body></html>`;
...
function ModuloRespuestas({ alumnos, tokens: tokensProp, setTokens, configId, cfg }) {
  ...
  const simularRespuestas = async () => {
    ...
    if (!window.confirm(`Â¿Simular respuestas para ${pendientes.length} alumnos pendientes?`)) return;

    setSimulando(true);
    const DIAS = ['lunes','martes','miercoles','jueves','viernes'];
    const FRANJAS = ['manana','mediodia','tarde']; // Sin noche: no se dan clases en esa franja

    // Peso: 70% disponible la mayorÃ­a de dÃ­as, 30% dÃ­as sueltos
    const genDias = () => {
      const dias = {};
      const nDias = Math.floor(Math.random() * 3) + 2; // 2-4 dÃ­as
    ...
    for (const tok of pendientes) {
      try {
        ...
        // Alumnos con transporte: mÃ¡x 2 dÃ­as
        const dias = {};
    ...
    await cargar();
    setSimulando(false);
    alert(`â ${ok} respuestas simuladas${err > 0 ? ` Â· â ï¸ ${err} errores` : ''}`);
  };
  ...
  const DIAS_L = { lunes:"Lun", martes:"Mar", miercoles:"MiÃ©", jueves:"Jue", viernes:"Vie", sabado:"SÃ¡b" };
  const FRANJAS_L = { manana:"MaÃ±ana", mediodia:"Mediodía", tarde:"Tarde" };
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <div style={{ fontSize:13, fontWeight:700, color:"#1A3A6B" }}>ð¬ Respuestas Â· {semanaLabel ? semanaLabel({}) : ""}</div>
<div style={{ display:"flex", gap:8 }}>
          <button
            ...
            {simulando ? "â³ Simulando..." : "ð² Simular respuestas"}
          </button>
          <button
      ...
      {pendientes.length > 0 && (
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:12, fontWeight:700, color:"#E65100", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:8 }}>â³ Pendientes de respuesta</div>
          {pendientes.map(t => {
            ...
            return (
              <div key={t.id} style={{ background:"#FFF8F0", borderRadius:10, border:"1.5px solid #FFE0B2", padding:"10px 14px", marginBottom:6, display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div>
                  <div style={{ fontSize:14, fontWeight:700 }}>{a.apellidos||"â"}, {a.nombre||"â"}</div>
                  <div style={{ fontSize:11, color:"#9A7A5A" }}>{a.localidad||""} Â· {a.permiso||""}</div>
                </div>
              </div>
            );
          })}
      ...
      {disponibilidades.length > 0 && (
        <div>
          <div style={{ fontSize:12, fontWeight:700, color:"#2E7D32", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:8 }}>â Disponibilidades recibidas</div>
            ...
            return (
              <div key={d.id} style={{ background:"white", borderRadius:12, border:"1.5px solid #C8E6C9", marginBottom:10, overflow:"hidden" }}>
                <div style={{ background:"#F1F8E9", borderBottom:"1px solid #C8E6C9", padding:"10px 14px", display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:14, fontWeight:700 }}>{a.apellidos||"â"}, {a.nombre||"â"}</div>
                    <div style={{ fontSize:11, color:"#5A7A5A" }}>{a.localidad||""} Â· {a.permiso||""}</div>
                  </div>
                ...
                <div style={{ padding:"10px 14px" }}>
                  {d.preferencia_horaria && (
                    <div style={{ fontSize:12, color:"#5A7A5A", marginBottom:8 }}>
                      ð Preferencia: <strong>{d.preferencia_horaria}</strong>
                    </div>
                  )}
                  {diasConFranjas.length === 0 ? (
                    <div style={{ fontSize:12, color:"#9A9A9A", fontStyle:"italic" }}>Sin franjas especÃ­ficas indicadas</div>
                  ) : (
      ...
      <button onClick={cargar} disabled={cargando} style={{ width:"100%", marginTop:8, padding:12, borderRadius:10, border:"1.5px solid #1A3A6B", background:cargando?"#F0F4FF":"white", color:"#1A3A6B", fontFamily:"inherit", fontSize:13, fontWeight:700, cursor:cargando?"not-allowed":"pointer" }}>
        {cargando ? "â³ Actualizando..." : "ð Actualizar ahora"}
      </button>
    </div>
  );
}
...
function ModuloWhatsApp({ alumnos, tokens, setTokens, configId }) {
  ...
  // Polling cada 15s para actualizar estado â/â³
  useEffect(() => {
    if (!configId) return;
  ...
  const genMensaje = (alumno, token) => {
    ...
    return `Hola ${alumno.nombre} ð\n\nTe enviamos el enlace para indicar tu disponibilidad de prÃ¡cticas *esta semana*:\n\nð ${enlace}\n\nâ° RellÃ©nalo antes de la fecha lÃ­mite indicada en el formulario.\n\n_Autoescuela Herrero Â· 688 70 86 69_`;
  };
        ...
        return (
          <div key={t.id} style={{ background:"white", borderRadius:12, border:"1.5px solid "+(respondido?"#C8E6C9":"#E8E0D5"), marginBottom:10, overflow:"hidden" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", background:respondido?"#F1F8E9":"#F7F3EE", borderBottom:"1px solid #F0EBE5" }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:14, fontWeight:700 }}>{a.apellidos}, {a.nombre}</div>
                <div style={{ fontSize:12, color:"#7A7A7A" }}>{a.localidad} Â· {a.permiso}</div>
              </div>
              <span style={{ fontSize:11, fontWeight:700, color:respondido?"#2E7D32":"#E65100", background:respondido?"#E8F5E9":"#FFF3E0", padding:"3px 8px", borderRadius:20 }}>
                {respondido?"â Enviado":"â³ Pendiente"}
              </span>
            </div>
            {!respondido && (
              <div style={{ padding:"10px 14px" }}>
  ...
  return (
    <div style={{ background:"white", borderRadius:14, border:"1.5px solid "+cp+"33", marginBottom:16, overflow:"hidden", pageBreakInside:"avoid" }}>
      <div style={{ background:cp, padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <div style={{ color:"white", fontWeight:800, fontSize:15 }}>ð¤ {PROF_LABEL[profKey]}</div>
        ...
        <div style={{ textAlign:"right" }}>
          ...
          <div style={{ color:"white", opacity:0.8, fontSize:10 }}>prÃ¡cticas</div>
            ...
            <div key={dia} style={{ marginBottom:10 }}>
              ...
              {pracs.map((p,i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 10px", background:"#F7F9FF", borderRadius:8, borderLeft:"3px solid "+cp, marginBottom:4 }}>
                  <div style={{ fontWeight:700, fontSize:13, minWidth:90, color:"#1A1A1A" }}>{p.desde} <span style={{fontWeight:400,color:"#888",fontSize:11}}>Â· {durLabel(p.duracion)}</span></div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:700, fontSize:13 }}>{p.alumnoNombre}</div>
                    <div style={{ fontSize:11, color:"#777", marginTop:1 }}>
                      <Badge color={COLOR_PERM[p.permiso]}>{p.permiso}</Badge>
                      {" "}{VEH_LABEL[p.vehiculo]||"â"} Â· {p.tipo==="pista"?"ð Pista":"ð£ï¸ CirculaciÃ³n"}
                    </div>
                  </div>
                </div>
              ))}
    ...
    <div style={{ background:"white", borderRadius:14, border:"1.5px solid #E8E0D5", marginBottom:16, overflow:"hidden" }}>
      <div style={{ background:"#C8102E", padding:"12px 16px", display:"flex", alignItems:"center", gap:12 }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ color:"white", opacity:0.85, fontSize:11, marginTop:2 }}>
            {alumno.localidad} Â· Permiso {alumno.permiso}{alumno.fase?" Â· "+alumno.fase:""}
            {alumno.transporte && <span style={{ marginLeft:6, background:"rgba(255,255,255,0.25)", borderRadius:8, padding:"1px 7px", fontSize:10, fontWeight:700 }}>ð Con transporte</span>}
          </div>
        </div>
        <div style={{ textAlign:"right" }}>
      ...
      <div style={{ padding:"10px 14px" }}>
            ...
            <div key={dia} style={{ marginBottom:8 }}>
              ...
              {pracs.map((p,i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 10px", background:"#F7F3EE", borderRadius:8, borderLeft:"3px solid #1A3A6B", marginBottom:4 }}>
                  <div style={{ fontWeight:700, fontSize:13, color:"#1A1A1A" }}>{p.desde} <span style={{fontWeight:500,color:"#888",fontSize:11}}>Â· {durLabel(p.duracion)}</span> <span style={{fontSize:11,color:"#555"}}>{p.tipo==="pista"?"ð Pista":"ð£ï¸ CirculaciÃ³n"}</span></div>
        ...
        <div style={{ marginTop:8, paddingTop:8, borderTop:"1px solid #F0EBE5", fontSize:10, color:"#999", textAlign:"center" }}>
          Autoescuela Herrero Â· C/ TenerÃ­as 6 bajo Â· Trujillo Â· 688 70 86 69
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background:"white", borderRadius:14, border:"1.5px solid #1A3A6B33", marginBottom:16, overflow:"hidden" }}>
      <div style={{ background:"#1A3A6B", padding:"12px 16px" }}>
        <div style={{ color:"white", fontWeight:800, fontSize:15 }}>ð Planning Semanal General</div>
        <div style={{ color:"white", opacity:0.85, fontSize:11, marginTop:2 }}>{semanaLabel(cfg)} Â· {totalPracs} prÃ¡cticas totales</div>
      </div>
            ...
            {PROFS.map(pk => {
              ...
              return (
                <tr key={pk}>
                  ...
                  <div style={{ fontWeight:700, fontSize:10, color:cp }}>{p.desde} <span style={{fontWeight:400,color:"#888"}}>Â· {durLabel(p.duracion)}</span></div>
...
// ââ PDF RUTA DE RECOGIDAS âââââââââââââââââââââââââââââââââ
function generarPDFRuta(planning, alumnos, cfg) {
  const durLabel = (min) => !min ? "" : min < 60 ? min + "min" : Math.floor(min/60) + "h" + (min%60 ? (min%60)+"min" : "");
  // Alumnos con transporte que tienen prÃ¡cticas esta semana
  ...
  // Orden fijo de localidades (de mÃ¡s lejos a mÃ¡s cerca de Trujillo o por orden habitual)
  ...
  // Generar HTML
  let html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
  <title>Ruta de recogidas â ${semanaLabel(cfg)}</title>
  <style>
  ...
  html += `<div class="header"><h1>ð Ruta de Recogidas</h1><p>${semanaLabel(cfg)} Â· Autoescuela Herrero Â· 688 70 86 69</p></div>`;
  ...
  for (const dia of DIAS_SEMANA) {
    ...
    html += `<table><tr><th>#</th><th>Hora</th><th>Localidad</th><th>Alumno</th><th>PrÃ¡ctica</th></tr>`;
    let prevLoc = null;
    paradasOrdenadas.forEach((p, i) => {
      if (p.localidad !== prevLoc) {
        if (prevLoc !== null) {
          html += `<tr class="loc-sep"><td colspan="5">ð ${p.localidad}</td></tr>`;
        }
        prevLoc = p.localidad;
      }
      const tipoBadge = p.tipo === 'pista'
        ? '<span class="tipo-badge pista">ð Pista</span>'
        : '<span class="tipo-badge circ">ð£ï¸ Circ.</span>';
      html += `<tr>
...
function ModuloInformes({ planning, alumnos, cfg, tokens, configId }) {
  ...
  if (!planning) return (
    <div style={{ textAlign:"center", padding:"60px 20px", color:"#7A7A7A" }}>
      ...
      <div style={{ fontSize:13 }}>Genera el planning desde la pestaÃ±a Planning para ver los informes.</div>
    </div>
  );
  ...
  const msgProfesor = (profKey) => {
    ...
    let msg = "Hola "+PROF_LABEL[profKey]+" ð\n\n*Planning "+semanaLabel(cfg)+"*\n\n";
    for (const dia of DIAS_SEMANA) {
      const dp = pracs.filter(p=>p.dia===dia);
      if (!dp.length) continue;
      msg += "*"+DIAS_LABEL[dia].toUpperCase()+"*\n";
      dp.forEach(p => { msg += "  "+p.desde+" ("+durLabel(p.duracion)+") â "+p.alumnoNombre+" ("+p.permiso+") "+VEH_LABEL[p.vehiculo]+"\n"; });
      msg += "\n";
    }
  ...
  const msgAlumno = (alumno) => {
    ...
    if (!pracs.length) return "";
    let msg = "Hola "+alumno.nombre+" ð\n\n*Tus prÃ¡cticas "+semanaLabel(cfg)+"*\n\n";
    for (const dia of DIAS_SEMANA) {
      const dp = pracs.filter(p=>p.dia===dia);
      if (!dp.length) continue;
      msg += "*"+DIAS_LABEL[dia].toUpperCase()+"*\n";
      dp.forEach(p => { msg += "  "+p.desde+" Â· "+( p.tipo==="pista"?"ð Pista":"ð£ï¸ CirculaciÃ³n")+"\n"; });
      msg += "\n";
    }
    msg += "_Autoescuela Herrero Â· 688 70 86 69_";
    return msg;
  };
  ...
  return (
    <div style={{ paddingBottom:180 }}>
      <div style={{ fontSize:16, fontWeight:800, color:"#1A3A6B", marginBottom:12 }}>ð Informes Â· {semanaLabel(cfg)}</div>

      {/* Selector de vista */}
      <div style={{ display:"flex", gap:6, marginBottom:16, overflowX:"auto" }}>
        {[{k:"profesores",l:"ð¤ Profesores"},{k:"alumnos",l:"ð Alumnos"},{k:"semanal",l:"ð Semanal"}].map(v=>(
      ...
      {vista==="alumnos" && (
        <div>
          {/* BotÃ³n ruta si hay alumnos con transporte */}
          {alumnos.some(a=>a.transporte && DIAS_SEMANA.some(d=>(planning[d]||[]).some(p=>p.alumnoId===a.id))) && (
            <div style={{ marginBottom:14, padding:"12px 14px", background:"#FFF3F3", borderRadius:10, border:"1.5px solid #C8102E", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <div>
                <div style={{ fontWeight:700, fontSize:13, color:"#C8102E" }}>ð Hay alumnos con transporte esta semana</div>
                <div style={{ fontSize:11, color:"#888", marginTop:2 }}>Genera el PDF de ruta con las paradas ordenadas</div>
              </div>
              <button onClick={()=>generarPDFRuta(planning,alumnos,cfg)} style={{ flexShrink:0, padding:"9px 18px", background:"#C8102E", color:"white", border:"none", borderRadius:20, cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:800 }}>ð PDF Ruta</button>
            </div>
          )}
      ...
      {vista==="semanal" && (
        <div>
          ...
          <button onClick={()=>imprimirSeccion("informe-semanal")}
            ...
            ð¨ï¸ PDF / Imprimir Planning Completo
          </button>
        </div>
      )}
    </div>
...
// ââââââââââââââââââââââââââââââââââââââââââââââ
const NAV = [
  { key:"config",      label:"Config",    icon:"âï¸" },
  { key:"alumnos",     label:"Alumnos",   icon:"ð¥" },
  { key:"respuestas",  label:"Respuestas",icon:"ð¬" },
  { key:"planning",    label:"Planning",  icon:"ð" },
  { key:"informes",    label:"Informes",  icon:"ð" },
...
export default function AppOficina() {
  ...
  if (!autenticado) return <LoginOficina onLogin={() => setAutenticado(true)} />;
  ...
  // ââ Handler reset nueva semana âââââââââââââââââââââââââââââ
  const handleNuevaSemana = () => {
    setCfg(configInicial());
    setConfigId(null);
    setAlumnos([]);
    setTokens([]);
    setPlanning(null);
      ...
      <div style={{ background:"white", padding:"12px 20px", borderBottom:"3px solid #1A3A6B", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 12px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ textAlign:"right" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <button onClick={()=>{ sessionStorage.removeItem(SESSION_KEY); setAutenticado(false); }} style={{ fontSize:10, color:"#9A9A9A", background:"none", border:"1px solid #DDD", borderRadius:8, padding:"3px 8px", cursor:"pointer", fontFamily:"inherit", fontWeight:600 }}>Salir</button>
            </div>
        </div>
      </div>
