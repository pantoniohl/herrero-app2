...
import supabase, { validarToken, marcarTokenUsado } from "./lib/supabase.js";
...
export default function AppAlumno() {
  ...
  useEffect(() => {
    if (!tokenStr) { setEstado("invalido"); return; }
    validarToken(tokenStr).then(res => {
      if (!res.valido) {
        setEstado(res.motivo === "ya_enviado" ? "ya_enviado" : "invalido");
        setTokenData(res);
      } else {
        setTokenData(res);
  ...
  const handleEnviar = async (disp, pracs) => {
    ...
    try {
      ...
      alert("Error al enviar disponibilidad: " + (e.message || e));
    } finally {
      setEnviando(false);
    }
  };
  ...
  // Construir fechasDias a partir de la config
  ...
  if (fechaDesde) {
    ...
    DIAS_SEMANA_ALU.forEach((d, i) => {
      const f = new Date(base);
      f.setDate(base.getDate() + i);
      fechasDias[d] = f.toLocaleDateString("es-ES", { weekday:"short", day:"numeric", month:"short" });
    });
  }

  return (
    <Formulario
      alumno={tokenData.alumno}
      fechaLimite={fechaLimite}
      semanaDesde={fechaDesde}
      semanaHasta={fechaHasta}
      fechasDias={fechasDias}
      onEnviar={enviando ? ()=>{} : handleEnviar}
    />
  );
}
