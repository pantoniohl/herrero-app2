...
// ══════════════════════════════════════════════
// Token leído de la URL: herrero-app2.vercel.app/alumno?token=xxx
...
function formatFechaLimite(fechaStr) {
  if (!fechaStr) return "";
  const d = new Date(fechaStr);
...
export default function AppAlumno() {
  ...
  const [estado, setEstado] = useState("cargando"); // cargando | invalido | ya_enviado | formulario | confirmado
  ...
  useEffect(() => {
    ...
    const validar = async () => {
      try {
        ...
        if (data.usado) { setTokenData({ alumno: data.alumnos }); setEstado("ya_enviado"); return; }
  ...
  if (estado === "cargando") return (
    <div style={{ minHeight:"100vh", background:"#F7F3EE", fontFamily:"system-ui, sans-serif", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ fontSize:15, fontWeight:600, color:"#1A3A6B" }}>Comprobando enlace...</div>
    </div>
  );

  if (estado === "invalido") return <PantallaEnlaceInvalido />;
  if (estado === "ya_enviado") return <PantallaYaEnviado alumno={tokenData.alumno} />;
  if (estado === "confirmado") return <PantallaConfirmacion alumno={tokenData.alumno} disponibilidad={dispGuardada} practicasDeseadas={pracGuardadas} />;

  const { alumno, config, tokenId } = tokenData;
  const semanaDesde = formatFecha(config.fecha_desde);
  const semanaHasta = formatFecha(config.fecha_hasta);
  const fechaLimite = formatFechaLimite(config.fecha_limite);
  const fechasDias = config.fechas_dias || {};
  ...
  const handleEnviar = async (disp, pracs) => {
    try {
      ...
      // Serializar disponibilidad (Sets no son JSON)
      const diasSerial = Object.fromEntries(
        Object.entries(disp).map(([d, v]) => [d, v instanceof Set ? { modo:"franjas", franjas:[...v] } : v])
      );
      await sb.from("disponibilidad").insert({
        token_id: tokenId,
        alumno_id: alumno.id,
        config_id: config.id,
        dias: diasSerial,
        practicas_deseadas: pracs,
      });
      await sb.from("tokens_alumno").update({ usado: true }).eq("id", tokenId);
  ...
  return <Formulario alumno={alumno} fechaLimite={fechaLimite} semanaDesde={semanaDesde} semanaHasta={semanaHasta} fechasDias={fechasDias} onEnviar={handleEnviar} />;
}
